...
% Using Mamdani inference for transparent decision-making
% Centroid defuzzification for balanced outputs
fis.DefuzzificationMethod = 'centroid'; 

% Verification
input = [12, 3]; % Test case: 12°C, Activity=3
output = evalfis(fis,input);
disp(['For input [12°C, Activity=3], Heater Power = ' num2str(output) '%']);

% Visualization (uncomment to view)
% figure; plotmf(fis,'input',1); title('Temperature Membership Functions');
% figure; plotmf(fis,'input',2); title('Activity Level Membership Functions');
% figure; gensurf(fis); title('Control Surface');
% ruleview(fis);